...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("שגיאה ביצירת החשבון. ייתכן והאימייל כבר קיים במערכת.");
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === "auth/popup-blocked") {
        setError("הדפדפן חסם את החלון הקופץ. אנא אשר חלונות קופצים בדפדפן.");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 dark:from-[#0c1222] dark:to-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden transition-colors duration-300" dir="rtl">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[40%] h-[40%] bg-blue-100/40 dark:bg-blue-600/10 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-[-15%] right-[-10%] w-[35%] h-[35%] bg-sky-100/40 dark:bg-sky-500/10 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="w-full max-w-[440px] animate-page-enter relative z-10">
        ...
        <div className="bg-white/80 dark:bg-slate-800/60 backdrop-blur-xl p-8 md:p-10 rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(15,23,42,0.06)] dark:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.3)] border border-slate-200/50 dark:border-slate-700/50 transition-colors duration-300">
          <div className="mb-8 text-center md:text-right">
            <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-1.5 font-display tracking-tight transition-colors">יצירת חשבון</h1>
            <p className="text-black dark:text-white font-bold text-sm uppercase tracking-wide transition-colors">הצטרפו למהפכת הלמידה והתרגול</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            >
              <span>{isLoggingIn ? "מעבד נתונים..." : "הרשמה למערכת"}</span>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs uppercase"><span className="px-4 bg-white dark:bg-slate-800 text-black dark:text-white font-black transition-colors">או הירשמו באמצעות</span></div>
          </div>
          ...
          <p className="mt-8 text-center text-[14px] font-bold text-black dark:text-white transition-colors">
            כבר יש לכם חשבון?{" "}
            <Link href="/login" className="text-blue-600 dark:text-blue-400 hover:underline hover:text-blue-700 dark:hover:text-blue-300 transition-colors">התחברו כאן</Link>
          </p>
        </div>
      </div>
    </div>
